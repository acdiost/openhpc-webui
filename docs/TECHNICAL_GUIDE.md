...
本文档适用于 `openhpc_webui 0.3.0`，面向负责安装、配置、维护和开发的集群管理员与开发者。终端用户请阅读[用户使用手册](./USER_MANUAL.md)。
...
应用日志按单行 JSON 输出到标准输出，`LOG_LEVEL` 控制最低级别，默认 `INFO`。每条记录包含 UTC 时间、级别、logger、消息和 `request_id`；每个响应也通过 `X-Request-ID` 返回相同标识，便于用户报告问题时关联服务端日志。普通读取请求只记录路径、状态、来源与耗时，不记录请求体或查询内容。

所有 `POST`、`PUT`、`PATCH`、`DELETE` API 请求都会生成 `event_type=audit` 的审计记录，包括操作者、来源 IP、动作、资源与目标、修改前后快照、HTTP 状态、结果和耗时。登录失败同样记录，但不会记录密码。密码、令牌、Cookie、Session、私钥、LDAP 管理凭据等字段在日志最终输出前统一替换为 `[REDACTED]`。

生产环境应由 Supervisor/systemd 或日志采集器持久化标准输出，并设置访问权限、轮转周期、保留期限和防篡改策略。只有受权审计人员可以读取日志；不要依赖应用日志保存接口返回的 SSH 私钥或作业输出内容。
