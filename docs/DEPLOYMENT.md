...
LOGIN_LIMITER_DB_PATH=/srv/openhpc-webui/.runtime/login_attempts.sqlite3
```
...
启动目录中的 `.env` 是安装向导、系统设置、终端设置和管理员权限的统一持久化文件；也可设置绝对路径 `OPENHPC_WEBUI_ENV_FILE` 指向其他可写文件。与 Supervisor 的服务环境保持同一文件，不要将凭据写入 Python 安装目录。更新版本后旧的登录会话需重新登录，LDAP 用户删除或重建也会使原会话失效。
...
文件管理中，普通用户的页面根目录固定映射到 LDAP `homeDirectory`；每次文件操作都在独立进程中以该用户的 NSS/SSSD UID、主 GID 和附加组执行，路径访问受 Home 范围及 Linux 文件权限共同限制。部署主机必须能唯一解析该用户且 LDAP UID/GID 与本机身份一致；无法降权时操作会被拒绝。管理员首次进入自己的 NSS/SSSD Home，但访问范围仍映射到系统 `/`，并沿用门户进程权限。要让管理员真正以 root 权限访问所有路径，Supervisor 必须按仓库默认配置使用 `user=root`；如果改为专用运行账户，管理员页面会提示权限受限。在线编辑仅接受大小不超过 `FILE_EDIT_MAX_KB` 的 UTF-8 普通文本。由于管理员可修改和删除系统路径，只应向受信任的运维人员授予管理员权限，并确保审计日志被集中留存。
...
终端页会显示默认模型，并允许用户通过“AI 配置”为当前 WebSocket 会话启用或关闭 AI，覆盖 Provider、Base URL、模型、API Key 和超时时间。用户还可以直接选择建议模型或手动输入模型 ID。会话级配置不修改 `.env`，也不影响其他用户；默认情况下，用户输入的 API Key 只保存在当前服务端会话内，不回显、不写入浏览器存储，连接关闭后随会话清除。

用户可选择把配置和 API Key 持久化到本地浏览器。前端使用 Web Crypto 生成随机 256 位、不可导出的 AES-GCM `CryptoKey`，加密后的配置、随机 IV 和密钥对象按登录用户名保存在当前 Origin 的 IndexedDB；新连接就绪后自动解密并通过 WSS 恢复到服务端会话。此功能只在安全上下文中启用，生产环境必须使用 HTTPS/WSS。取消持久化或清除站点数据会删除本地配置。由于同源脚本仍可请求浏览器使用不可导出的密钥执行解密，此机制提供的是静态存储加密，不能抵御 XSS、恶意浏览器扩展或已控制终端设备的攻击；部署时仍应配置严格的 CSP、保护静态资源供应链并使用受管终端。

只有 Provider 和 Base URL 均未改变时，留空才会沿用当前连接已有的密钥；切换端点会清除旧密钥，避免向新服务发送原密钥。未保存浏览器配置时，重连后恢复全局默认值；已保存时会在连接后自动应用本地配置。切换配置或模型会清除 AI 上下文、目标、待确认动作和自动批准状态，但不会重启 Shell。若兼容服务承载多个模型，应在模型服务侧配置访问控制和资源配额。
