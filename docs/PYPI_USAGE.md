...
本文档适用于从 PyPI 安装 `openhpc-webui 0.3.1`，而不是从 Git 仓库部署。
...
```bash
sudo install -d -m 0755 /srv/openhpc-webui
sudo python3 -m venv /srv/openhpc-webui/.venv
sudo /srv/openhpc-webui/.venv/bin/python -m pip install --upgrade pip
sudo /srv/openhpc-webui/.venv/bin/python -m pip install "openhpc-webui==0.3.1"
```

验证安装结果：

```bash
/srv/openhpc-webui/.venv/bin/python -c \
  'import openhpc_webui; print(openhpc_webui.__version__)'
/srv/openhpc-webui/.venv/bin/python -m pip show openhpc-webui
```
...
```bash
sudo chown root:root /srv/openhpc-webui/.env
sudo chmod 600 /srv/openhpc-webui/.env
```
...
```bash
set -a
. /srv/openhpc-webui/.env
set +a
/srv/openhpc-webui/.venv/bin/openhpc_webui
```
...
User=root
Group=root
WorkingDirectory=/srv/openhpc-webui
EnvironmentFile=/srv/openhpc-webui/.env
ExecStart=/srv/openhpc-webui/.venv/bin/uvicorn openhpc_webui.application:app --host 127.0.0.1 --port 6827 --proxy-headers --forwarded-allow-ips=127.0.0.1
Restart=on-failure
RestartSec=5
...
```bash
sudo cp -a /srv/openhpc-webui/.env /srv/openhpc-webui/.env.bak
sudo /srv/openhpc-webui/.venv/bin/python -m pip install --upgrade \
  "openhpc-webui==0.3.1"
sudo systemctl restart openhpc-webui
```

升级到后续版本时，将命令中的 `0.3.1` 替换为目标版本号。
...
```bash
python3 -m pip download --dest wheelhouse "openhpc-webui==0.3.1"
```

将整个 `wheelhouse` 目录复制到内网主机后安装：

```bash
/srv/openhpc-webui/.venv/bin/python -m pip install \
  --no-index --find-links ./wheelhouse "openhpc-webui==0.3.1"
```
...
确认服务读取了 `/srv/openhpc-webui/.env`，并且 `SECRET_KEY` 至少包含 32 个字符：
...
sudo /srv/openhpc-webui/.venv/bin/python -m pip uninstall openhpc-webui
```

卸载 Python 包不会删除 `/srv/openhpc-webui/.env`、systemd unit、Nginx 配置、LDAP
数据或 Slurm 配置。
